...
| 用户需要 GPT-M 2.0 生成截图再设计 / 信息图 / 证据墙 | B 也很合适(S22 主图、S15/S16 图片格是默认承载方式) |
...
如果主题是 AI / 工程 / 产品方法论 / 技术实践,不要只做“高级但空”的观点页。写 HTML 前先给每页列一行 slide brief:

```text
页码 → 一句话主张 → 例子/证据 → 现场讲法 → 下一页过渡
```

硬规则:
- 每页只讲一个主张,但必须能回答“我上台这一页要讲什么”
- 抽象概念页必须配一个具体任务、代码/工具/流程例子,或一个可执行判断标准
- 不要用生造中文词硬译英文;逐页朗读一遍,不像自然中文就改
- 如果一页只有几个漂亮名词,没有例子、证据或讲法,拆掉重写
...
如果当前运行环境是 **Codex**,完成 deck 初稿后,主动问用户是否需要用 GPT-M 2.0 / GPT-Image 2.0 生成配图并插入 PPT。不要默认生成。若当前环境没有可用的图片生成工具,必须明确说明,不要把手写 SVG / HTML 信息图说成 GPT 生成图。
...
- 瑞士风 S22 信息图默认使用独立页眉栏,不要把 `chrome-min` / 标题块压在图上;只有照片主视觉且显式 `data-s22-overlay-ok="true"` 才允许覆盖
...
- 行内 SVG 只负责几何图形。外部 SVG/PNG 配图可以有必要短标签,但不能包含 deck 级标题、页眉、页脚、页码或重复 HTML 标题。
...
**默认不要让图片 `align-self:end`**——会滑到页面底部,很容易碰到分页组件。用 grid 容器 + `align-items:start`(template 已预设)让图片贴顶即可;如果显式使用实验图文版式,必须确认模板已经内置 `--nav-safe-bottom` 安全区。
...
22. **S15/S16 图片同组一致**——同一组图片统一比例、高度、边距、线条粗细;信息图/UI 图加 `.fit-contain`
23. **组件角色要正确**——S22 的说明和三列证据点必选;数据专用版式必须有真实数据,不能靠文案硬填
24. **通用/非通用版式要分清**——P3/P8/P11/P19 较通用;P6/P7/P20/P21 是数据专用;P14/P15/P17 是结构专用;S22 是图片/案例/信息图专用
